        ...
        <div className="chatContent">
          ...
          <form onSubmit={onMessageSubmit}>
            ...
            <div className='name-field'>
              <textarea className="Chat-input" name='message' onKeyPress={(event => {
                if(event.key ==='Enter') {
                  onMessageSubmit(event)
                }
              } )} onChange={e => onTextChange(e)} value={state.message} placeholder={'Message'}></textarea>
              <button className="Chat-input__submit"><SendLogo /></button>
            </div>
          </form>
        </div>
      )}
