...
const SingleProduct = () => {
    ...
    useEffect(() => {
      axios.get(`${process.env.REACT_APP_API_URL}/products/get-one/`,{params: paramsId})
            ...
            <div className='single-product__details'>
                <OWLcorusel
                    ...
                    //   autoplay
                      ...
                      margin={20}
                    >
                        <img src={`${process.env.REACT_APP_API_URL}/uploads/product_images/${singleItem.image}`}/>
                        <img src={`${process.env.REACT_APP_API_URL}/uploads/product_images/${singleItem.image}`}/>
                </OWLcorusel>
                <div className='single-product__details-content'>
                    <h1 className='single-product__details-title'>Product Details</h1>
                    <h2 className='single-product__details-subtitle'>{singleItem.name}</h2>
                    <p className='single-product__details-text'>{singleItem.description}</p>
                </div>
            </div>
            <div className='single-product__content'>
                <div className='single-product__sale'>25% OFF</div>
                <h2 className='single-product__content-title'>{singleItem.name}</h2>
                <p className='single-product__content-category'>{singleItem.product_category[0].name}</p>
                <div className='single-product__content-box'>
                    <p className='single-product__content-price'>Product Price <span className='prevPrice'>&#36;{singleItem.normal_price}</span></p>
                    <p className='single-product__content-price'>Selling Price <span className='salePrice'>&#36;{singleItem.sales_price}</span></p>
                    <p className='single-product__content-price'>Available in:    <span className='productKg'>300g</span></p>
                </div>
